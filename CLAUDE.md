...
Nightly canary (`.github/workflows/canary.yml`) clones the latest
[wows-render-gamedata](https://github.com/toalba/wows-render-gamedata) tag
and runs the full test suite against it. On failure it opens (or appends to)
...
The repo is public so no auth secret is needed.
...
binary schemas at runtime from the [wows-render-gamedata](https://github.com/toalba/wows-render-gamedata)
...
- **Hardcoded constants**: ribbon IDs / consumable type IDs / weapon-type enums live directly in parser source (`ribbons.py`, `consumable_calc.py`, `battle_results.py`). When a new patch shifts an enum, update the source table — there is no JSON to load.
...
The `wows-gamedata/` subdir (local clone name; upstream is
[github.com/toalba/wows-render-gamedata](https://github.com/toalba/wows-render-gamedata))
is a public sibling repo regenerated by an upstream extraction pipeline whenever
WoWs ships a new build. It carries only the production-required subset — no
decompiled WG sources or extracted-constant JSONs.
...
Note: ribbon IDs / consumable type IDs / weapon-type enums and other constants
that previously lived in `extracted_constants.json` are now hardcoded in the
parser source (see `ribbons.py:RIBBON_WIRE_IDS`, `consumable_calc.py:CONSUMABLE_TYPE_ID_MAP`,
etc.). When a new patch shifts an enum, update the hardcoded table.
